...
const total_rows = ref(production.materials ? production.materials.length : 0);
const rows = ref(props.rows ? toRaw(props.rows) : [])
total_rows.value = rows.value.length;

watch(() => production.materials, (newMaterial) => {
    rows.value = toRaw(newMaterial)
    total_rows.value = newMaterial.length;
...
const exportTable = (type) => {
    ...
    const filename = 'Materials_' + new Date().toISOString().slice(0, 10) + '_' + new Date().getTime();
