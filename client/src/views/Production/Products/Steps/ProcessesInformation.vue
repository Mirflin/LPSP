...
const options = ref([])
const process_counter = ref(1)
console.log(props.form.processes)
onMounted(async() => {
    loading.value = true
...
const proceses = props.form.processes ? props.form.processes : ref([{'id': process_counter.value, 'process': '', 'subprocess': ''}])

const newProccess = () => {
    process_counter.value += 1
    proceses.value.push({'id': process_counter.value, 'process': '', 'subprocess': ''})
}
...
const process_form = ref({
    "processes": proceses.value
})
...
<template>
    <div v-if="!loading" class="rounded-2xl border border-gray-200 bg-white h-auto p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
        ...
        <div v-for="process in proceses">
            <ProcessesInput :process_list="production.processes" v-model="proceses" :process="process" />
